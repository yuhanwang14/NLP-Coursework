...
{\LARGE\bfseries Detecting Patronizing and Condescending Language\\[0.3em]
in News Paragraphs}

\vspace{1.5cm}

{\Large NLP Coursework 2026}

\vspace{1cm}

{\large SemEval-2022 Task 4, Subtask 1 — Binary PCL Classification}

\vspace{2cm}

\begin{tabular}{rl}
    \textbf{Leaderboard Name:} & \texttt{TODO\_LEADERBOARD} \\[0.5em]
    \textbf{GitHub Repository:} & \url{https://github.com/TODO/NLP} \\[0.5em]
...
\begin{enumerate}[leftmargin=*]
    ...
    The paper evaluates several baselines---SVM with TF-IDF/Word2Vec,
    BiLSTM, and transformer models (BERT, RoBERTa, DistilBERT)---on both
    tasks, establishing that RoBERTa achieves the best F1 of 0.706 on binary
    detection. This provides a reference point for subsequent research.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
    ...
    Only 9.4\% of paragraphs (995/10,637) are labelled as PCL, yet the
    authors do not experiment with standard imbalance-handling
    techniques such as oversampling, class-weighted losses, or focal
    loss. The benchmark results may thus understate what is achievable
    with proper imbalance handling.
...
RoBERTa-base baseline (F1 $\approx $ 0.48 on the SemEval dev split) by a
...
\textbf{Actual result:} Dev F1 = \textbf{0.575}, a +0.095 improvement over
...
Repository link: \url{https://github.com/TODO/NLP}
...
\midrule
Baseline (RoBERTa-base) & 0.480 & 0.490 \\
\textbf{Our model (RoBERTa-large)} & \textbf{0.575} & \textbf{TBD}$^*$ \\
\bottomrule
\end{tabular}
...
The comparison reveals that the majority of examples are correctly
classified by both models (``Both correct''). Critically, the number of
cases where our model is correct and the baseline is wrong substantially
exceeds the reverse, confirming the improvement is genuine and
broad-based rather than coming from a few lucky predictions.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Repository:} \url{https://github.com/TODO/NLP}
    \item \textbf{Leaderboard name:} \texttt{TODO\_LEADERBOARD}
