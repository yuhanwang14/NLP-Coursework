...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Extended training with regularisation:} Training for up to
          8 epochs with early stopping (patience 3 on internal validation
          F1) allows the model to converge properly while preventing
          overfitting. Crucially, we hold out 15\% of the training data
          as an internal validation split for model selection, ensuring
          that the official dev set remains a clean held-out evaluation.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \caption{Precision-recall curve with optimal threshold marked.
             The optimal F1-maximising threshold is 0.901, far above
             the default 0.5.}
    \label{fig:pr}
\end{figure}
...
(maximising F1) is \textbf{0.901}, yielding F1 = 0.592 compared to 0.575
at the default 0.5 boundary. The fact that the optimal threshold is so
high indicates that the model's softmax probabilities are poorly
calibrated: it assigns high confidence ($>$0.9) to most of its positive
predictions, meaning a much higher threshold is needed to filter out
uncertain cases. This suggests that probability calibration techniques
(e.g.\ temperature scaling) could be beneficial. Note that our submitted
\texttt{test.txt} predictions use the default 0.5 threshold, as the
optimal threshold was determined post-hoc on the dev set and applying it
to test predictions would risk overfitting to the dev distribution.
